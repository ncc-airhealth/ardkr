...
`pipeline`이 발행하는 open 버킷 static STAC JSON이 빌드 산출물이다.
손으로 직접 고치지 않는다.
...
- description은 웹에서 읽히는 글임. (읽기 쉽게 쓰되, 분석에 필요한 사실은 빼지 않음)
- 맨 위에는 데이터의 정체를 제목 없이 짧게 작성. (필요할 때만 처리 요지를 덧붙임)
- 원본이 설명하지 않는 내용은 추정하지 않음
- `## 처리`에는 파이프라인이 데이터에 가한 변환·수정만 기록 (출처 충돌·필드 해석은 넣지 않음)
- `## 주의사항`에는 `../inspect-data-quality/SKILL.md` 조사 결과 중 필드·link로 담기 어렵고, 해석·분석·재사용 판단을 바꾸는 것만 남긴다. 뜻이 겹치면 한 항목으로 묶는다.
